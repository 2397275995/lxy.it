...
## 小红书爬取失败 - 账号异常 (code: 300011)
Q: 爬取小红书时出现错误：`检测到账号异常，请稍后重启试试 (code: 300011)` 或 `status: 461`？<br>
A: 这是小红书的风控机制，常见原因和解决方案如下：<br>

**主要原因：**
1. **账号被风控**：账号行为异常，触发了小红书的安全检测
2. **Cookie失效**：登录状态过期或无效
3. **请求频率过高**：短时间内发送过多请求
4. **签名参数错误**：请求签名（X-S, X-T等）生成不正确
5. **IP被限制**：当前IP地址被小红书限制

**解决方案：**
1. **等待一段时间**：账号被风控后，通常需要等待几小时到几天才能恢复
2. **重新登录**：
   - 删除 `browser_data/cdp_xhs_user_data_dir` 文件夹
   - 使用 `--lt qrcode` 重新扫码登录
   - 确保在浏览器中能正常访问小红书
3. **降低爬取频率**：
   - 在 `config/base_config.py` 中增加 `CRAWLER_MAX_SLEEP_SEC` 的值（建议设置为 3-5 秒）
   - 减少 `CRAWLER_MAX_NOTES_COUNT` 的值，避免一次性爬取太多
4. **更换账号**：使用其他小红书账号进行爬取
5. **检查网络环境**：确保网络连接稳定，考虑使用代理IP
6. **手动验证**：在浏览器中手动访问小红书，完成任何验证码或安全验证

**预防措施：**
- 合理控制爬取频率，避免短时间内大量请求
- 使用真实浏览器环境，避免被识别为自动化工具
- 定期更新Cookie，保持登录状态有效
- 遵守平台规则，不要进行大规模爬取<br>
